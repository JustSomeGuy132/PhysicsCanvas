...
XMFLOAT3 BoundingShape::GetMaxPoint() {
	switch (type) {
	case Cuboid:
		XMFLOAT3 maxP = {};
		for (XMFLOAT3 vert : CuboidVertices()) {
...
XMFLOAT3 BoundingShape::GetMinPoint() {
	switch (type) {
	case Cuboid:
		XMFLOAT3 minP = {};
		for (XMFLOAT3 vert : CuboidVertices()) {
...
bool BoundingShape::PointCollidingWithObject(XMFLOAT3 point, std::shared_ptr<BoundingShape> object) {
	switch (object->type) {
	case Cuboid:
...
bool BoundingShape::IsColliding(std::shared_ptr<BoundingShape> first, std::shared_ptr<BoundingShape> other) {
	if (first->type == BoundType::Cuboid) {
		//cube-cube collisions
		if (other->type == BoundType::Cuboid) {
			for (XMFLOAT3 v1 : first->CuboidVertices()) {
				if (PointCollidingWithObject(v1, other))
					return true;
			}
			for (XMFLOAT3 v2 : other->CuboidVertices()) {
		...
		//cube-sphere collisions
			...
			return (PointCollidingWithObject(XMFLOAT3(other->position.x, other->position.y + other->dimensions.y, other->position.z), first)
				|| PointCollidingWithObject(XMFLOAT3(other->position.x, other->position.y - other->dimensions.y, other->position.z), first)
				|| PointCollidingWithObject(XMFLOAT3(other->position.x + other->dimensions.x, other->position.y, other->position.z), first)
				|| PointCollidingWithObject(XMFLOAT3(other->position.x - other->dimensions.x, other->position.y, other->position.z), first)
				|| PointCollidingWithObject(XMFLOAT3(other->position.x, other->position.y, other->position.z + other->dimensions.z), first)
				|| PointCollidingWithObject(XMFLOAT3(other->position.x, other->position.y, other->position.z - other->dimensions.z), first)
				);
		}
	}
		...
		if (other->type == BoundType::Cuboid) {
			return (PointCollidingWithObject(XMFLOAT3(first->position.x, first->position.y + first->dimensions.y, first->position.z), other)
				|| PointCollidingWithObject(XMFLOAT3(first->position.x, first->position.y - first->dimensions.y, first->position.z), other)
				|| PointCollidingWithObject(XMFLOAT3(first->position.x + first->dimensions.x, first->position.y, first->position.z), other)
				|| PointCollidingWithObject(XMFLOAT3(first->position.x - first->dimensions.x, first->position.y, first->position.z), other)
				|| PointCollidingWithObject(XMFLOAT3(first->position.x, first->position.y, first->position.z + first->dimensions.z), other)
				|| PointCollidingWithObject(XMFLOAT3(first->position.x, first->position.y, first->position.z - first->dimensions.z), other)
				);
		}
...
std::vector<XMFLOAT3> BoundingShape::ResolveCollisions(std::shared_ptr<BoundingShape> first, std::shared_ptr<BoundingShape> other) {
	XMFLOAT3 half(first->dimensions.x / 2.f, first->dimensions.y / 2.f, first->dimensions.z / 2.f);
	XMFLOAT3 oHalf(other->dimensions.x / 2.f, other->dimensions.y / 2.f, other->dimensions.z / 2.f);

	XMFLOAT3 min1(-half.x, -half.y, -half.z);

	min1 = PhysMaths::Float3Add(first->position, min1);min1 = PhysMaths::RotateVector(min1, first->rotation);

	XMFLOAT3 max1(half.x, half.y, half.z);

	max1 = PhysMaths::Float3Add(first->position, max1);max1 = PhysMaths::RotateVector(max1, first->rotation);

	XMFLOAT3 min2(-oHalf.x, -oHalf.y, -oHalf.z);

	min2 = PhysMaths::Float3Add(other->position, min2);min2 = PhysMaths::RotateVector(min2, other->rotation);

	XMFLOAT3 max2(oHalf.x, oHalf.y, oHalf.z);
	...
	XMFLOAT3 trans1 = XMFLOAT3();
	XMFLOAT3 trans2 = XMFLOAT3();
...
std::vector<CuboidFace> BoundingShape::CuboidFaces() {
	assert(type == BoundType::Cuboid && "Not a cuboid!!");
...
XMFLOAT3 BoundingShape::CuboidFaceCentre(CuboidFace face) {

	return XMFLOAT3((face.vert1.x + face.vert2.x + face.vert3.x + face.vert4.x) / 4.0f,
		(face.vert1.y + face.vert2.y + face.vert3.y + face.vert4.y) / 4.0f,
		(face.vert1.z + face.vert2.z + face.vert3.z + face.vert4.z) / 4.0f);
}

XMFLOAT3 BoundingShape::CuboidFaceNormal(CuboidFace face) {
	assert(type == BoundType::Cuboid && "Not a cuboid!");
	XMFLOAT3 posToFac(CuboidFaceCentre(face).x - position.x, CuboidFaceCentre(face).y - position.y, CuboidFaceCentre(face).z - position.z);

	XMFLOAT3 vec1(face.vert2.x - face.vert1.x, face.vert2.y - face.vert1.y, face.vert2.z - face.vert1.z);
	XMFLOAT3 vec2(face.vert4.x - face.vert1.x, face.vert4.y - face.vert1.y, face.vert4.z - face.vert1.z);

	XMFLOAT3 normal = PhysMaths::Float3Cross(vec1, vec2);
...
std::vector<XMFLOAT3> BoundingShape::ContactPointsTo(std::shared_ptr<BoundingShape> obj2) {
	std::vector<XMFLOAT3> ret;
	if (type == BoundType::Cuboid) {
		...
		for (Edge edge : edges) {
			...
			//if only the first vertex is in contact with the object
				...
				XMFLOAT3 dir(edge.vert2.x - edge.vert1.x, edge.vert2.y - edge.vert1.y, edge.vert2.z - edge.vert1.z);
				XMFLOAT3 point = edge.vert1;
				XMFLOAT3 point2;
			...
			//if only the second vertex is in contact
				...
				if (!alreadyContained)
					ret.push_back(edge.vert2);
				//Traverse along this edge to see where it stops being in contact, registering that point as a contact point
				XMFLOAT3 dir(edge.vert1.x - edge.vert2.x, edge.vert1.y - edge.vert2.y, edge.vert1.z - edge.vert2.z);
				XMFLOAT3 point = edge.vert2;
				XMFLOAT3 point2;
...
XMFLOAT3 BoundingShape::ClosestPointOn(std::shared_ptr<BoundingShape> obj2) {
	switch (obj2->GetType()) {
	case BoundType::Cuboid:

		break;
	case BoundType::Sphere:
		XMFLOAT3 dir(obj2->position.x - position.x, obj2->position.y - position.y, obj2->position.z - position.z);
		dir = PhysMaths::VecTimesByConstant(dir, obj2->dimensions.x / PhysMaths::Magnitude(dir));
		return XMFLOAT3(obj2->position.x + dir.x, obj2->position.y + dir.y, obj2->position.z + dir.z);
		break;
	}
}
